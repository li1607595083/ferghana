...
//                        .build()));
        ...
        dbTableEnv.executeSql("CREATE TABLE oracle_dim ("
                ...
                + ") WITH ("
                + "'connector' = 'jdbc',"
                + "'url' = 'jdbc:oracle:thin:@192.168.100.101:1521:helowin',"
...
//        String sql_join = "SELECT  TRADE_ID, COUNT(TRADE_ID) OVER(PARTITION BY TRADE_ID ORDER BY TRAN_TIME RANGE BETWEEN INTERVAL '30' MINUTE preceding AND CURRENT ROW) AS AMS FROM main_oracle";
        ...
        dbTableEnv.executeSql("CREATE TABLE sink_oracle ("
                + "TRADE_ID STRING,"
                + "TRADE_AMOUNT STRING,"
                + "PRIMARY KEY (TRADE_ID) NOT ENFORCED "
                + ") WITH ("
                + "'connector' = 'jdbc',"
                + "'url' = 'jdbc:oracle:thin:@192.168.100.101:1521:helowin',"
                + "'table-name' = 'SINK_FLINK_ORACLE_TEST_001',"
                + "'username' = 'LOGMINER',"
                + "'password' = 'LOGMINER',"
                + "'driver' = 'oracle.jdbc.driver.OracleDriver')");
        StatementSet statementSet = dbTableEnv.createStatementSet();
        statementSet.addInsertSql("INSERT INTO sink_oracle " + sql_join);
