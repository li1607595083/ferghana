...
public class FlinkUtils {
    ...
    public static StreamTableEnvironment dbTableEnv(StreamExecutionEnvironment bsEnv, Properties parameterProperties) {
        ...
        if (parameterProperties.getProperty(ParameterName.RUM_MODE) != null && parameterProperties.getProperty(ParameterName.RUM_MODE).equals(RunMode.START_MODE)){
            if (parameterProperties.getProperty(ParameterName.VARIABLE_PACK_EN) == null){
                System.err.println("ERROR: 变量包名字不能为空！");
                System.exit(-1);
            } else {
                streamTableEnvironment.getConfig().setJob_name(parameterProperties.getProperty(ParameterName.VARIABLE_PACK_EN));
            }
        }
