...
public class ParameterUtils {
    ...
    /**
     * @desc  对 MYSQL CDC 添加输出类型字段
     * @param properties 参数属性
     */
    private static void sinkSqlDeal(Properties properties){
        if (properties.getProperty(ParameterName.SINK_SQL) != null && SourceType.MYSQL_CDC.equals(properties.getProperty(ParameterName.SOURCE_TYPE))){
            // 如果是 MYSQL CDC 需要添加数据类型字段
    ...
    /**
     * @desc 删除字符串中指定符号(最后一位)的前面部分
     * @param str 目标字符串
     * @param symblo 指定符号
     * @param isSpecialSymbol 是否为特殊符号
     * @return
     */
    public static String removeBeforeLastSpecialSymbol(String str, String symblo, boolean isSpecialSymbol){
        // 先反转
        String reverse = StringUtils.reverse(str);
        // 指定分割符号
        String separator = isSpecialSymbol ? String.format("\\%s", symblo) : symblo;
        // 获取结果
        String result = reverse.split(separator, 2)[0];
        return StringUtils.reverse(result);
    }
    ...
    private static String changeKafkaSourceConfig(String sql, Properties properties, int parameter) throws Exception {
        if (properties.getProperty(ParameterName.RUM_MODE).equals(RunMode.TEST_MODE)){
            ...
            String testTopic = "test_" + properties.getProperty(ParameterName.TEST_TOPIC_NAME) + parameter;
            // 数据源表，表名
    ...
    private static void  sourceTableSqlDeal(Properties properties) throws Exception {
        ...
        if (sourceType.equals(SourceType.TWO_STREAM_JOIN) || sourceType.equals(SourceType.ONE_STREAM)){
            for (int i = 0; i < sourceSet.length; i++) {
                // 修改 kafka 的配置参数
                String createSql = changeKafkaSourceConfig(sourceSet[i], properties, i);
    ...
    private static String[] dealMysqlCdc(Properties properties, String sql) {
        // CREATE TABLE MYSQL_CDC_1(...) WITH (...)|[U. I, D]
    ...
    private static String getTestRedisTableSql(String sql, Properties properties){
        String[] schema = sql.split("WITH", 2);
        if (schema[1].contains("'" + ParameterConfigName.REDIS_HASHNAME + "'")){
            properties.put(ParameterConfigName.REDIS_HASHNAME, getSingMeta(sql, "hashname"));
        }
        String first = removeBeforeFirstSpecialSymbol(schema[1], "(", true);
        String sencond = removeTailLastSpecialSymbol(first, ")", true);
        List<String> strings = Arrays.asList(sencond.split("','"));
        for (int i = 0; i < strings.size(); i++) {
            if (strings.get(i).startsWith("password")){
                strings.remove(strings.get(i));
            }
            if (strings.get(i).startsWith("database")){
                strings.remove(strings.get(i));
            }
            if (strings.get(i).startsWith("cluster-nodes")){
                strings.set(i, strings.get(i).replaceFirst("cluster-nodes", "single-node"));
            }

        }
        String init = "(" + StringUtils.join(strings, "','") + ")";
        String redisNodes = getSingMeta(init, "single-node");
        String delaResult = init.replaceFirst("'" + redisNodes + "'", "'" + properties.getProperty(ParameterName.TEST_REDIS_NODE) + "'");

        return schema[0] + "WITH" + delaResult;
    ...
    private static String changeDimensionSql(String sql, String type, int counts, Properties properties){
        if (DimensionType.DIM_REDIS.equals(type)){
            sql = getTestRedisTableSql(sql, properties);
        } else if (DimensionType.DIM_MYSQL.equals(type) || DimensionType.DIM_ORACLE.equals(type)) {
            ...
            if (DimensionType.DIM_ORACLE.equals(type)) {
                sql = sql.replace("'" + tableNmae + "'", "'" + (properties.getProperty(ParameterName.TEST_TOPIC_NAME) + counts).toUpperCase() + "'")
    ...
    private static void dimTableSqlDeal(Properties properties){
        ...
        if (dimensionTable != null){
            for (Object oj : JSONObject.parseArray(dimensionTable).toArray()) {
                ...
                hashMap.put(ParameterName.DIMENSION_TABLE_SQL, dimensionSql);
                arrayList.add(JSONObject.toJSON(hashMap).toString());
                dimCounts++;
            }
