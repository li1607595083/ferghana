...
/**
 * @param <T>
 * @description 对 public void deserialize(ConsumerRecord<byte[], byte[]> message, Collector<T> out) throws Exception {.....} 进行了修改
 * 添加了主键字段，对从 kafka 消费的数据偏移量
 */
@Internal
...
//        deserializationSchema.deserialize(message.value(), out);
        deserializationSchema.deserialize(message.value(), out,"primary_key" ,"p" + message.partition() + "_" + message.offset());
    }
