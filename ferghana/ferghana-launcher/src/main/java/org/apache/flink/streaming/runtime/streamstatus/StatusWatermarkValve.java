...
/**
 * @desc 增加了成员变量，数据源的乱序时间（timeout），以及双流 join 的乱序时间，以及上次发送的最大 watermark,
 *  真个类的改动不大，但改动的方法比较多，比如 inputStreamStatus（）这个状态方法改为只试用于数据
 */
@Internal
public class StatusWatermarkValve {

    public  boolean isSourec = false;
    private long timeout;
    private long twostreamjoindelaytime;
    private final DataOutput output;
    private long lastMaxWaterMark;
