...
package org_change.org.apache.flink.streaming.connectors.kafka;
...
public abstract class FlinkKafkaConsumerBase<T> extends RichParallelSourceFunction<T> implements
    ...
    public final void snapshotState(FunctionSnapshotContext context) throws Exception {
        if (!running) {
            ...
            topic_partition_offset_state_copy.clear();
            ...
            if (fetcher == null) {
                ...
                for (Map.Entry<KafkaTopicPartition, Long> subscribedPartition : subscribedPartitionsToStartOffsets.entrySet()) {
                    ...
                    topic_partition_offset_state_copy.add(
                            subscribedPartition.getKey().getTopic() + "\t" +
                                    subscribedPartition.getKey().getPartition() + "\t" +
                                    subscribedPartition.getValue().toString());
                }
                ...
                for (Map.Entry<KafkaTopicPartition, Long> kafkaTopicPartitionLongEntry : currentOffsets.entrySet()) {
                    ...
                    topic_partition_offset_state_copy.add(
                            kafkaTopicPartitionLongEntry.getKey().getTopic() + "\t" +
                                    kafkaTopicPartitionLongEntry.getKey().getPartition() + "\t" +
                                    kafkaTopicPartitionLongEntry.getValue().toString());
