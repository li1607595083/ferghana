 ...
 * @desc 新增成员变量 input2Statue，以及对 processWatermark1， processWatermark2 方法进行了修改
 */
@PublicEvolving
public abstract class AbstractStreamOperator<OUT>
    ...
    // 新增成员变量, 双流 join 的时候，会保存左边流的状态
    private int input2Statue = 0;
    private int input1Statue = 0;
    ...
    /**
     * @desc 根据右边流的状态，来确定发出的 waterMark，且将当前流的状态发送到下游;
     * @param mark
     * @throws Exception
     */
    public void processWatermark1(Watermark mark) throws Exception {
        input1Watermark = mark.getTimestamp();
        input1Statue = mark.getTdlease();
        long newMin = input2Statue == 0 ? Math.min(input1Watermark, input2Watermark) : input1Watermark;
        if (newMin > combinedWatermark) {
            combinedWatermark = newMin;
            if (input1Statue == -1 && combinedWatermark == input1Watermark){
                processWatermark(new Watermark(combinedWatermark, input1Statue));
            } else {
