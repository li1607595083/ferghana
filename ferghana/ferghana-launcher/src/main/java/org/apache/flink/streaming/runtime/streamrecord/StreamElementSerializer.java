...
package org_change.org.apache.flink.streaming.runtime.streamrecord;
...
public final class StreamElementSerializer<T> extends TypeSerializer<StreamElement> {
    ...
    public void serialize(StreamElement value, DataOutputView target) throws IOException {
        if (value.isRecord()) {
            ...
            target.writeInt(value.asWatermark().getTdlease());
            target.writeBoolean(value.asWatermark().isTwostreamjoin());
        }
        else if (value.isStreamStatus()) {
            target.write(TAG_STREAM_STATUS);
            target.writeInt(value.asStreamStatus().getStatus());
            target.writeLong(value.asStreamStatus().getTimeout());
            target.writeLong(value.asStreamStatus().getTwostreamjoindelay());
            target.writeBoolean(value.asStreamStatus().isSource());
        }
    ...
    public StreamElement deserialize(DataInputView source) throws IOException {
        ...
        if (tag == TAG_REC_WITH_TIMESTAMP) {
            ...
            StreamRecord<T> tStreamRecord = new StreamRecord<>(typeSerializer.deserialize(source), timestamp);
            return tStreamRecord;
        }
        else if (tag == TAG_REC_WITHOUT_TIMESTAMP) {
            StreamRecord<T> tStreamRecord = new StreamRecord<>(typeSerializer.deserialize(source));
            return tStreamRecord;
        }
        else if (tag == TAG_WATERMARK) {
            return new Watermark(source.readLong(), source.readInt(), source.readBoolean());
        }
        else if (tag == TAG_STREAM_STATUS) {
            return new StreamStatus(source.readInt(), source.readLong(), source.readLong(), source.readBoolean());
        }
        else if (tag == TAG_LATENCY_MARKER) {
