...
package org_change.org.apache.flink.metrics.influxdb;
    ...
    private BatchPoints buildReport() {
        ...
        try {
            for (Map.Entry<Gauge<?>, MeasurementInfo> entry : gauges.entrySet()) {
                report.point(org.apache.flink.metrics.influxdb.MetricMapper.map(entry.getValue(), timestamp, entry.getKey()));
            }
