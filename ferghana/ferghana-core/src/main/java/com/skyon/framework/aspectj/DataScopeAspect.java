...
public class DataScopeAspect {
     ...
     * @param alias     别名
     */
    public static void serviceTableScope(JoinPoint joinPoint, SysUser user) {
        ...
        for (SysRole role : user.getRoles()) {
            ...
            if (DATA_SCOPE_ALL.equals(dataScope)) {
                ...
                sqlString.append(StringUtils.format(" OR t.create_id IN (SELECT u.user_id FROM sys_user u " +
                        " LEFT JOIN sys_role_dept d ON u.dept_id = d.dept_id " +
                        " WHERE  d.role_id = {})", role.getRoleId()));
            } else if (DATA_SCOPE_DEPT.equals(dataScope)) {
                sqlString.append(StringUtils.format(" OR t.create_id IN (SELECT u.user_id FROM sys_user u " +
                        " WHERE u.dept_id = {})", user.getDeptId()));
            } else if (DATA_SCOPE_DEPT_AND_CHILD.equals(dataScope)) {
                sqlString.append(StringUtils.format(" OR t.create_id IN (SELECT u.user_id FROM sys_user u " +
                        " LEFT JOIN sys_dept d on u.dept_id = d.dept_id " +
                        " WHERE d.dept_id = {} or find_in_set( {} , ancestors ))", user.getDeptId(), user.getDeptId()));
            } else if (DATA_SCOPE_SELF.equals(dataScope)) {
                sqlString.append(StringUtils.format(" OR t.create_id IN({})", user.getUserId()));
            }
        }
        if (StringUtils.isNotBlank(sqlString.toString())) {
            BaseEntity baseEntity = (BaseEntity) joinPoint.getArgs()[0]; // 获取切入点的参数
