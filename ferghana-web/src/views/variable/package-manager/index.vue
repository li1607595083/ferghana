          ...
          <el-dialog :title="title" :visible.sync="open" customClass="customWidth" :close-on-click-modal="false"
            ...
            <el-tabs v-model="activeName" type="card">
              ...
              <el-tab-pane label="测试结果" name="second">
                <div class="table">
                  <el-table :data="testResultData" tooltip-effect="dark" border stripe style="width: 100%">
                    <template v-for='(col,index) in testResultCol'>
                      <el-table-column :show-overflow-tooltip="true" align="center" :prop="col.dataItem"
                        :label="col.dataName" :key="index" width="200px">
                      </el-table-column>
                    </template>
                  </el-table>
                </div>
      ...
      <el-form ref="resourceForm" :model="resourceForm" :rules="rules" class="el-col-24">
        <div class="el-col-24">
          <el-form-item class="el-col-24" prop="radio" label="配置类型：">
            ...
            <el-radio v-model="resourceForm.radio" label="radio3" :disabled="radio3Disable" @change="radioChange">
              使用上次资源配置</el-radio>
          </el-form-item>
        </div>
...
<script>
  ...
  import {
    listPackageOperate
  } from "@/api/variable/variablePackageOperate";
    ...
    data() {
      return {
        ...
        rules: {
          ...
          jobMemory: [{
            required: true,
            message: "jobManager堆内存不能为空",
            trigger: "blur"
          }],
          taskMemory: [{
            required: true,
            message: "taskManager堆内存不能为空",
            trigger: "blur"
          }],
          variableId: [{
            required: true,
      ...
      testRun() {
        this.$refs["form"].validate(valid => {
          if (valid) {
              ...
              for (let j = 0; j < parse1.length; j++) { // 循环第一个变量的维表字段
                second: for (let l = 0; l < parse1[j].col.length; l++) { // 循环维表里的字段 id phone name
                  ...
                  if (flag1 === true) { // 新增一个维表
                    ...
                    let dmo2 = [];
                    dmo2.push({});
                    Object.assign(this.listResultDimension[this.listResultDimension.length - 1], {
                      col: dmo
                    });
                    Object.assign(this.listResultDimension[this.listResultDimension.length - 1], {
                      dimensionTableValue: dmo2
                    }); // 增加输入框
                    break second;
                  }
            ...
            for (let i = 0; i < this.sourceTableCol.length; i++) {
              if (this.sourceTableCol[i].dataName.indexOf("主键") > 0) {
                this.form.sourceKey = this.sourceTableCol[i].dataName.substr(0, this.sourceTableCol[i].dataName
                  .length - 3)
                break;
              }
            }
          ...
          if (this.variableCenter[1].children != null && this.variableCenter[1].children.length > 0) {
            for (let j = 0; j < this.variableCenter[1].children.length; j++) {
              ...
              if (this.form.variableId.indexOf(id) > 0 && variable.indexOf(node.id) > 0) {
                ...
                this.$alert("该基础变量被【" + label + "】派生变量所引用，请先删除【" + label + "】", {
                  type: "error"
                });
              }
            }
        ...
        axios({
          ...
          for (let i = 0; i < resp.data.rows.length; i++) {
            for (let key in resp.data.rows[i]) {
              ...
              // 变量因子等等的字段
              Object.assign(that.listDimension[i], {
                dimensionRelationOptions: s
              });
              // 自定义的选项字段
              Object.assign(that.listDimension[i], {
                defineDimensionFieldItem: s
              });
              ...
              // 条件选项的字段
              Object.assign(that.listDimension[i], {
                dimensionStatisticsfieldOptions: s2
              });
            }
          }
        }).catch(resp => {
